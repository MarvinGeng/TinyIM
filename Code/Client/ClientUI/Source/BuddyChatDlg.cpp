...
/**  
 *   
 * @brief 响应添加消息到对应的富文本编辑框，在此处完成消息类型的分发
 * 		  "/f["系统表情id"]/s["系统表情index"]/c["自定义图片路径"]"
 * 
 * @param hRichEditWnd  富文本编辑框Handle,
 * @param lpText 具体的消息内容
 */
 ...
 * @param lpBuddyMsg 
 */
void CBuddyChatDlg::AddMsgToRecvEdit(C_UI_BuddyMessage* lpBuddyMsg)
{
	...
	if(!lpBuddyMsg->IsShakeWindowMsg() && nMsgType!= E_UI_CONTENT_TYPE::CONTENT_TYPE_IMAGE_CONFIRM)
	{
		CString strText;
		RichEdit_SetSel(m_richRecv.m_hWnd, -1, -1);
	...
	for (int i = 0; i < (int)lpBuddyMsg->m_arrContent.size(); i++)
	{
		...
		if (lpContent != NULL)
		{
            ...
            case E_UI_CONTENT_TYPE::CONTENT_TYPE_REMOTE_DESKTOP:
                {
                    int k = 0;
                }
                break;
			...
			if( lpContent->m_nType!= E_UI_CONTENT_TYPE::CONTENT_TYPE_FONT_INFO && nMsgType!= E_UI_CONTENT_TYPE::CONTENT_TYPE_IMAGE_CONFIRM)
			{
 ...
 * @param arrContent 
 */
//void CBuddyChatDlg::AddMsgToMsgLogEdit(std::vector<C_UI_Content*>& arrContent)
//{
//	//！！字体信息必须放在文本信息的前面
 ...
 * @param arrContent 
 */
//void CBuddyChatDlg::AddMsgToRecvEdit(std::vector<C_UI_Content*>& arrContent)
//{
//	//TODO 和AddMsgToMsgLogEdit 有部分代码重复，考虑是否可以合并
//	//！！字体信息必须放在文本信息的前面
//	static C_UI_FontInfo fontInfo;
//	for (long i = 0; i < (long)arrContent.size(); i++)
//	{
