...
LRESULT CMultiChatDlg::OnRichEdit_Recv_Link(LPNMHDR pnmh)
{
	if (pnmh->code == EN_LINK)
	{
		...
		if (pLink->msg == WM_LBUTTONUP)
		{
			...
			{
				C_UI_GroupInfo* pGroupInfo = /*GetGroupInfoPtr()*/NULL;
				if(pGroupInfo == NULL)
					return 0;
...
void CMultiChatDlg::OnMenu_ViewInfo(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	//if(m_lpFMGClient->m_UserMgr.IsFriend(nUTalkUin) || m_lpFMGClient->m_UserMgr.m_UserInfo.m_uUserID==nUTalkUin)
		::PostMessage(m_hMainDlg, WM_SHOW_BUDDY_INFO_DLG, 0, nUTalkUin);
	//else
	{
...
BOOL CMultiChatDlg::UpdateGroupMemberList()
{
	...
	UINT uAccountID = 0;
	E_UI_ONLINE_STATUS nStatus = E_UI_ONLINE_STATUS::STATUS_ONLINE;
	for (std::set<UINT>::iterator iter=m_setTargetIDs.begin(); iter!=m_setTargetIDs.end(); ++iter)
	{
		...
		if(IsGroupTarget(uAccountID))
		{
			...
			nStatus = E_UI_ONLINE_STATUS::STATUS_ONLINE;
		...
		if(nStatus != E_UI_ONLINE_STATUS::STATUS_OFFLINE && nStatus!= E_UI_ONLINE_STATUS::STATUS_INVISIBLE)
		{
			bGray = FALSE;
			++nOnlineMemberCnt;
		}
...
//void CMultiChatDlg::AddMsgToRecvEdit(C_UI_BuddyMessage* lpGroupMsg)
//{
...
void CMultiChatDlg::AddMsgToRecvEdit(std::vector<C_UI_Content*>& arrContent)
{
	//！！字体信息必须放在文本信息的前面
