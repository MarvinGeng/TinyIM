...
void CLogonUserInfoDlg::OnSysHead(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	changePicHeadDlg.SetSelection(m_userMgr.m_UserInfo.m_nFace);
...
void CLogonUserInfoDlg::OnCustomHead(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	CString strDestPath;
	CString strDumyPath;
	//strDestPath.Format(_T("%s%u.png"), m_userMgr.GetCustomUserThumbFolder().c_str(), m_userMgr.m_UserInfo.m_uUserID);
	...
	pFileItemRequest->m_nFileType = E_UI_FILE_ITEM_TYPE::FILE_ITEM_UPLOAD_USER_THUMB;
...
void CLogonUserInfoDlg::OnBtn_OK(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	UpdateLogonUserInfo(strNickName, 
		                strSignature,
		                uGender, 
		                nBirthday, 
		                strAddress, 
...
LRESULT CLogonUserInfoDlg::OnUploadUserThumbResult(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	E_UI_UPLOAD_USER_THUMB_RESULT eResult = static_cast<E_UI_UPLOAD_USER_THUMB_RESULT>(wParam);
	if(eResult == E_UI_UPLOAD_USER_THUMB_RESULT::UPLOAD_USER_THUMB_RESULT_FAILED)
	{
		::MessageBox(m_hWnd, _T("上传头像失败！"), g_strAppTitle.c_str(), MB_OK|MB_ICONERROR);
		
	}
	else if(eResult == E_UI_UPLOAD_USER_THUMB_RESULT::UPLOAD_USER_THUMB_RESULT_SUCCESS)
	{
...
void CLogonUserInfoDlg::UpdateCtrlData()
{
	...
	if(m_bUseCustomThumb)
	{
		...
		if(!Hootina::CPath::IsFileExist(strThumbPath))
		{
			...
			strThumbPath.Format(_T("%sImage\\UserThumbs\\%d.png"), g_szHomePath,m_userMgr.m_UserInfo.m_nFace);
			memset(m_szCustomFaceRemotePath, 0, sizeof(m_szCustomFaceRemotePath));
		}
	}
	else
	{
		strThumbPath.Format(_T("%sImage\\UserThumbs\\%d.png"), g_szHomePath, m_userMgr.m_UserInfo.m_nFace);
		m_uSysFaceID = m_userMgr.m_UserInfo.m_nFace;
	}
	
	m_picHead.SetBitmap(strThumbPath);

	m_edtNickName.SetWindowText(m_userMgr.m_UserInfo.m_strNickName.c_str());

	m_edtSign.SetWindowText(m_userMgr.m_UserInfo.m_strSign.c_str());

	long nGender = m_userMgr.m_UserInfo.m_nGender;
	if(nGender != 0)
	{
	...
	long nBirthday = m_userMgr.m_UserInfo.m_nBirthday;
	...
	m_edtAddress.SetWindowText(m_userMgr.m_UserInfo.m_strAddress.c_str());

	m_edtPhone.SetWindowText(m_userMgr.m_UserInfo.m_strMobile.c_str());

	m_edtMail.SetWindowText(m_userMgr.m_UserInfo.m_strEmail.c_str());
...
void CLogonUserInfoDlg::UpdateLogonUserInfo(CString strNickName,
	...
	BOOL bUseCustomThub)
{
	m_userMgr.m_UserInfo.m_strMarkName = strNickName;
	m_userMgr.m_UserInfo.m_nGender = uGender;
	m_userMgr.m_UserInfo.m_nBirthday = uBirthday;
	m_userMgr.m_UserInfo.m_strAccount = strNickName;
	m_userMgr.m_UserInfo.m_strAddress = strAddress;
	m_userMgr.m_UserInfo.m_strSign = strSignature;
	m_userMgr.m_UserInfo.m_nFace = m_uSysFaceId;
	m_userMgr.m_UserInfo.m_strEmail = strMail;
	m_userMgr.m_UserInfo.m_strMobile = strPhone;
	m_userMgr.m_UserInfo.m_strNickName = strNickName;
}