...
//		return;
	...
	if (nMsgId == 0)
	{
		...
		if (lpMsgList != NULL)
		{
			C_UI_MessageSender* lpMsgSender = lpMsgList->GetMsgSender(E_UI_CHAT_MSG_TYPE::FMG_MSG_TYPE_SESS, nUTalkUin);
			if (lpMsgSender != NULL)
			{
				int nMsgCnt = lpMsgSender->GetMsgCount();
				for (int i = 0; i < nMsgCnt; i++)
				{
					C_UI_SessMessage* lpSessMsg = lpMsgSender->GetSessMsg(i);
					if (lpSessMsg != NULL)
					{
						AddMsgToRecvEdit(lpSessMsg);
					}
				}
				lpMsgList->DelMsgSender(E_UI_CHAT_MSG_TYPE::FMG_MSG_TYPE_SESS, nUTalkUin);
				::PostMessage(m_hMainDlg, WM_DEL_MSG_SENDER,static_cast<WPARAM>(E_UI_CHAT_MSG_TYPE::FMG_MSG_TYPE_SESS), nUTalkUin);
			}
		}
	}
	else
	{
		...
		if (lpMsgList != NULL)
		{
			...
			if (lpMsgSender != NULL)
			{
				C_UI_SessMessage* lpSessMsg = lpMsgSender->GetSessMsgById(nMsgId);
				if (lpSessMsg != NULL)
				{
					AddMsgToRecvEdit(lpSessMsg);
...
{
	::PostMessage(m_hMainDlg, WM_SHOW_GMEMBER_INFO_DLG, m_nGroupCode, m_nUTalkUin);
}
...
// 获取群信息指针
C_UI_GroupInfo* CSessChatDlg::GetGroupInfoPtr()
{
...
// 获取好友信息指针
C_UI_BuddyInfo* CSessChatDlg::GetBuddyInfoPtr()
{
	//if (m_lpFMGClient != NULL)
	{
...
// 获取用户信息指针
C_UI_BuddyInfo* CSessChatDlg::GetUserInfoPtr()
{
	//if (m_lpFMGClient != NULL)
	{
...
void CSessChatDlg::AddMsgToRecvEdit(C_UI_SessMessage* lpSessMsg)
{
	...
	{
		C_UI_Content* lpContent = lpSessMsg->m_arrContent[i];
		if (lpContent != NULL)
		{
...
void CSessChatDlg::CloseMsgLogBrowser()
{
	...
	{
		::PostThreadMessage(m_dwThreadId, WM_CLOSE_MSG_LOG_DLG, 0, 0);
		m_dwThreadId = NULL;
	}
}