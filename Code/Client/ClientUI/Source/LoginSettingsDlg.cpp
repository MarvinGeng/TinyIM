...
BOOL CLoginSettingsDlg::InitUI()
{
	...
	//隐藏部分选项
	{
		this->m_editFileSrvAddr.ShowWindow(HIDE_WINDOW);
		this->m_editFilePort.ShowWindow(HIDE_WINDOW);
		this->m_editImgSrvAddr.ShowWindow(HIDE_WINDOW);
		this->m_editImgPort.ShowWindow(HIDE_WINDOW);
		this->m_editProxyAddr.ShowWindow(HIDE_WINDOW);
		this->m_editProxyPort.ShowWindow(HIDE_WINDOW);
		this->m_comboProxyType.ShowWindow(HIDE_WINDOW);
	}
	return TRUE;
}
...
void CLoginSettingsDlg::OnBtn_OK(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	{
		std::string strServerIp = EncodeUtil::UnicodeToUtf8(m_szSrvAddr);
		int serverPort = std::atoi(EncodeUtil::UnicodeToUtf8(m_szSrvPort));
