...
HttpClient g_client("127.0.0.1:8000");
UserRegisterRspMsg HttpUserRegister(const UserRegisterReqMsg& reqMsg)
{
	UserRegisterRspMsg rspMsg;
	try {
		auto rsp = g_client.request("POST", "/register_user", reqMsg.ToString());
...
UserUnRegisterRspMsg HttpUserUnRegister(const UserUnRegisterReqMsg& reqMsg)
{
	...
	try {
		auto rsp = g_client.request("POST", "/un_register_user", reqMsg.ToString());
		std::string strRsp = rsp->content.string();
		rspMsg.FromString(strRsp);
	}
	catch (const SimpleWeb::system_error& e) {
